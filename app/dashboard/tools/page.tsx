...
export default function ToolsScreen() {
    ...
    const toggleBudget = async (budget: Budget) => { setWorkingId(`budget-${budget.id}`); try { await BudgetsAPI.update(budget.id, { name: budget.name, period: "monthly", category: budget.category, limit_amount: budget.limit_amount, currency: "INR", alert_threshold_percent: budget.alert_threshold_percent, active: !budget.active }); await loadPlanning(); } catch (requestError) { setBudgetError(requestError); } finally { setWorkingId(null); } };
    const deleteBudget = async (budget: Budget) => { if (!window.confirm(`Delete ${budget.name}?`)) return; setWorkingId(`budget-${budget.id}`); try { await BudgetsAPI.delete(budget.id); await loadPlanning(); } catch (requestError) { setBudgetError(requestError); } finally { setWorkingId(null); } };
