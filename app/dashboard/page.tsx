                ...
                <section className="relative overflow-hidden rounded-[2rem] bg-zinc-950 p-6 text-white shadow-2xl shadow-zinc-900/10 sm:p-8 lg:p-10">
                    ...
                    <div className="relative flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
                        <div className="max-w-2xl">
                            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-bold text-white/70"><CalendarDays className="h-3.5 w-3.5 text-accent" /> {dashboard ? formatDateRange(dashboard.period.start, dashboard.period.end) : "Your money, in context"}</div>
                        ...
                        <section className="grid gap-6 xl:grid-cols-[1.4fr_1fr]">
                            <div className="rounded-[2rem] border border-border bg-white p-6 dark:bg-zinc-900 sm:p-8">
                                ...
                                <div className="mt-7 h-[300px] min-w-0">
                                    <ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} layout="vertical" margin={{ left: 4, right: 8 }}><CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#eee" /><XAxis type="number" hide /><YAxis dataKey="name" type="category" width={100} axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "#71717a" }} /><Tooltip formatter={(value) => formatMoney(Number(value))} cursor={{ fill: "rgba(255,136,101,.06)" }} contentStyle={{ borderRadius: 16, border: "1px solid #f0e5e7" }} /><Bar dataKey="amount" fill="#FF8865" radius={[0, 8, 8, 0]} barSize={24} /></BarChart></ResponsiveContainer>
                                </div>
                            </div>
                            <div className="rounded-[2rem] border border-border bg-white p-6 dark:bg-zinc-900 sm:p-8">
