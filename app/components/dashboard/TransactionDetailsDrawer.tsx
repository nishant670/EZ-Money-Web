                ...
                <div className="fixed inset-0 z-[110] flex justify-end">
                    ...
                    >
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-10">
                            ...
                            <div className="text-center space-y-4">
                                <div className="w-20 h-20 bg-accent/10 rounded-[2rem] flex items-center justify-center text-accent mx-auto shadow-sm">
                                    {transaction.merchant ? transaction.merchant[0] : <Wallet className="w-8 h-8" />}
                                </div>
                                <div>
                                    <h4 className="text-2xl font-bold font-rounded">{transaction.merchant}</h4>
                                    <p className="text-sm text-zinc-400 font-medium">{transaction.category}</p>
                                </div>
                                <h5 className={cn("text-4xl font-bold font-rounded", transaction.amount.startsWith("+") ? "text-green-500" : "dark:text-white")}>
                                    {transaction.amount}
                                </h5>
                            </div>
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                <div className="bg-zinc-50 dark:bg-zinc-800/50 p-6 rounded-3xl space-y-4">
                                    ...
                                    <div className="flex items-center justify-between text-xs font-bold uppercase tracking-widest text-zinc-400">
                                        ...
                                        <span className="text-zinc-900 dark:text-white">{transaction.account}</span>
                                    </div>
                                    <div className="flex items-center justify-between text-xs font-bold uppercase tracking-widest text-zinc-400">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-accent/5 border border-accent/20 p-6 rounded-3xl relative overflow-hidden group">
                                        ...
                                        <p className="text-xs font-medium text-zinc-500 leading-relaxed italic">
                                            "Extracted from ICICI SMS alert. Merchant identified as {transaction.merchant}. Category {transaction.category} assigned with 98% confidence."
                                        </p>
                                        <div className="mt-4 flex items-center gap-4">
                                            <div className="flex-1 h-1.5 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                                                <div className="h-full bg-accent w-[98%] rounded-full shadow-sm" />
                                            </div>
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 block ml-2">Payment Notes</label>
                                    <div className="bg-zinc-50 dark:bg-zinc-800 p-6 rounded-3xl text-sm text-zinc-600 dark:text-zinc-400 font-medium leading-relaxed">
                                        Transaction reference: #TXN-99201-MM-88. Paid via UPI linked to ICICI account. No further notes.
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-8 border-t border-border bg-zinc-50 dark:bg-zinc-800/50 space-y-3">
                            <button className="w-full flex items-center justify-center gap-3 bg-zinc-900 dark:bg-white dark:text-zinc-900 text-white py-4 rounded-2xl font-bold text-sm shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all">
                                <Copy className="w-4 h-4" /> Duplicate Transaction
                            </button>
                            <button className="w-full flex items-center justify-center gap-3 border border-red-500/20 text-red-500 py-4 rounded-2xl font-bold text-sm hover:bg-red-500/5 transition-all">
